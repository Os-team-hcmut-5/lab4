...
void* worker_loop(void* arg) {
    ...
    while (1) {
        ...
        pthread_mutex_lock(&queue_mutex);
        next_job->finish_time = current_sim_time; // Clock thread updates this continuously
...
int main(int argc, char* argv[]) {
    ...
    double throughput = (double)total_jobs / current_sim_time; // [cite: 142]
    double utilization = ((double)total_worker_busy_time / (num_workers * current_sim_time)) * 100.0; // [cite: 144]
